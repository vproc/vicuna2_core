...
module vproc_lsu import vproc_pkg::*; #(
    ...
    generate
        if (BUF_REQUEST) begin
            ...
            assign state_req_ready = (~state_req_valid_q & xif_mem_if.mem_ready) | (xif_mem_if.mem_valid & xif_mem_if.mem_ready) | (~state_req_stall & ~xif_mem_if.mem_valid & xif_mem_if.mem_ready);
        end else begin
            always_comb begin
                state_req_valid_q = state_req_valid_d;
                state_req_q       = state_req_d;
